...
def has_cycle(link):
    ...
    while fast is not Link.empty and fast.rest is not Link.empty:
        slow = slow.rest          # move slow pointer one step
        fast = fast.rest.rest     # move fast pointer two steps    
...
def reverse_other(t):
    ...
    def helper(t, depth):
        if depth % 2 == 0:
            reverse_label = [branch.label for branch in t.branches]
            reverse_label.reverse()
            for branch in t.branches:
                branch.label = reverse_label[0]
                reverse_label.pop(0)
        for branch in t.branches:
