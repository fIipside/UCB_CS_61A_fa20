...
def berry_finder(t):
    ...
    if is_leaf(t):
        return False
    return not all([not berry_finder(branch) for branch in branches(t)])
...
def riffle(deck):
    ...
    for index in range(len(deck)):
        shuffled_deck += ([first_half[index]] + [second_half[index]])
...
def add_trees(t1, t2):
    ...
    for i in range(min(len(b1), len(b2))):
        new_branch = add_trees(b1[i], b2[i])
        result_branches += new_branch
        i += 1
    result_branches += b1[i:]
    result_branches += b2[i:]
...
def shakespeare_tokens(path='shakespeare.txt', url='https://web.archive.org/web/20200814055112/http://composingprograms.com/shakespeare.txt'):
