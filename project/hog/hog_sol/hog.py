...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while k <= num_rolls:
        ran = dice()
        if ran == 1: # Don't use dice() == 1. Therefore you are calling it another time!!!
            i = 1
        sum += ran
        k += 1
    if i:
...
def swine_align(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4a
    k = player_score
    while k >= 10 and min(player_score, opponent_score) > 0:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # END PROBLEM 5
    # BEGIN PROBLEM 6
        say = say(score0, score1) # What is 'say' exactly is? See in hog_pui.py or in the Project 1_ The Game of Hog.html.
    # END PROBLEM 6
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who:
            last_score_new = score1
            if running_high < score1 - last_score:
                running_high_new = score1 - last_score
                print(running_high_new, 'point(s)! The most yet for Player', who)
            else:
                running_high_new = running_high
        else:
            last_score_new = score0            
            if running_high < score0 - last_score:
                running_high_new = score0 - last_score
                print(running_high_new, 'point(s)! The most yet for Player', who)       
            else:
                running_high_new = running_high
        return announce_highest(who, last_score_new, running_high_new)
    return say
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    """
    """
    In this particular case, *args doesn't carry any arguments because dice() 
    does not need any. However, *args is still defined in make_averaged to allow 
    flexibility in handling functions that do require arguments.
    """
    # BEGIN PROBLEM 8
    def new_fun(*args):
        k, sum = 1, 0
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    """
    Remember to just call roll_dice once in a loop!!!
    """
    # BEGIN PROBLEM 9
    num_rolls, i = 1, 1
    max_record = 0
    while i <= 10:
        cur_aver = make_averaged(roll_dice, trials_count)(i, dice)
        if cur_aver > max_record:
            num_rolls = i
            max_record = cur_aver
        i += 1
    return num_rolls  
...
def final_strategy(score, opponent_score):
    ...
    This strategy optimizes when the score is near 100, but failed.
    So I just modified the formal parameter. After some experiments. I think it is 
    advisable to use the catchup effect.
    """
    # BEGIN PROBLEM 12
    if opponent_score - score > 0 and opponent_score - score < 4:
        return 6
    return extra_turn_strategy(score, opponent_score, 10, 4)
    # return 6  # Replace this statement
    # END PROBLEM 12
