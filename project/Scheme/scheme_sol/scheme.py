...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operand = rest.map(lambda expr: scheme_eval(expr, env))
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        return self.env.make_child_frame(self.formals, args) # env just for testing? see PROBLEM 18 mu
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        return target
        # END PROBLEM 5
        # In terms of syntax, when Scheme parses define, it parses the right side (+ x 2) as a "list containing a single element" instead of a "single expression". For consistency, when Scheme parses (define (f x) (+ x 2)), it wraps (+ x 2) into a Pair, where first is an expression and rest is nil, indicating that this define statement has only one function body.
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(value) or expressions.rest is nil:
            return value
        expressions = expressions.rest
    return is_false_primitive(scheme_eval(expressions, env, True))
    # END PROBLEM 12
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        """Make a frame that binds my formal parameters to ARGS, a Scheme list
        of values, for a dynamically-scoped evaluated in environment ENV."""
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        # BEGIN PROBLEM 19
        """
        The core is to create new Thunk instances to store the calculated results. In this way, there is no need to use frames.
        ...
        def factorial_tail(n, acc):
            if n == 0:
                ...
                return Thunk(lambda: factorial_tail(n - 1, n * acc), env)

        def optimized_eval(expr, env):
            while isinstance(expr, Thunk):
                expr = expr.expr()  # Evaluate the Thunk lazily
            return expr

        e.g. calculate the factorial of factorial(3,1): factorial(3,1)->factorial(2,3)->factorial(1,6)
        At this time, no new frame is needed, because the results are already in the second parameter, and can be returned directly. tail = True indicates that tail recursion is currently required.
        When it is tail recursion and it is not an atomic expression, a Thunk instance is returned.

        The essence is to change to iteration. Enter the loop - call the original eval - find the tail context - call the new eval - return Thunk - end this call, space recovery - continue the loop.
        When he encounters an expression, he first calls optimized_eval and then when he enters, he will get a Thunk. Call original_scheme_eval for this thing. During the eval process,
        if a tail call is encountered, Thunk is returned directly. At this time, the while loop starts for the second and third times, etc. to save frames.
        If not, then continue original_scheme_eval based on the result of calling original_scheme_eval once.
        """
